...
class ProductsController extends Controller
{
	...
	{
        if ($id!='all') {
             $catSelected = Category::find($id);
            if ($catSelected->parent_id == 0) {
                $pro = Products::join('category', 'products.cat_id', '=', 'category.id')
                ->select('products.*')->where('category.parent_id',$catSelected->id)
                ->paginate(10);             
            } else {
                $pro = Products::where('cat_id', $catSelected->id)->paginate(10);
            }
    ...
    public function getadd($id)
    {
		...
		$cat= $p_id != 0 ? Category::where('parent_id',$p_id)->get() : Category::where('parent_id',$loai->id)->get();
        // dd($cat, $p_id);
		$pro = Products::all();	
        // if ($p_id >=19) {
        //         return view('back-end.products.pc-add',['data'=>$pro,'cat'=>$cat,'loai'=>$p_name->name]); thêm mới cho pc
        //     }
        // else {
            return view('back-end.products.add',['data'=>$pro,'cat'=>$cat,'parrentLoai'=>$p_name->name ?? null, 'loai' => $loai->name, 'idLoai' => $loai->id]);
        // }	
